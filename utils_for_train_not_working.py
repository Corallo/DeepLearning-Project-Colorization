...
def getYgivenZ(Z, w=64, h=64, Q=313, T=0.38):
    ...
    print(np.dot(ft,colorsList).shape)
    Y=np.dot(ft,colorsList).reshape(w,h,2)
    return Y

def generateImg(Z,light,w=64,h=64):
    Y = getYgivenZ(Z,w,h)
    newImg = np.zeros((224,224,3))
    prtin(newImg.shape)
    newImg[:,:,0]=light
    print(light.shape)
    #Something wrong here, maybe the resize should be 1,224,224, I will try after dinner
    newImg[:,:,1:]=cv2.resize(Y,(224,224))
    print(cv2.resize(Y,(224,224)).shape)
